...
RSpec.describe Analysers::UniqViewsAnalyser do
  describe '#generate_analysis' do
    ...
    it 'returns analysis with correct title' do
      expect(analysis).to have_attributes(title: 'uniq views')
    end

    it 'returns analysis with correct log entries' do
      expect(analysis.log_entries).to contain_exactly(an_object_having_attributes(path: '/path_a', visitor_counter: 2),
